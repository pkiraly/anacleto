 ...
 *
 */
public class TeiProcessor extends SourceTypeHandler {
    ...
    public void indexBook(Book book) 
		...
		FastTeiIndexer indexer;
		try {
			File xsl = book.getTei2IndexingStyleSheet();
			indexer = new FastTeiIndexer(xsl);
	...
	public ContentBean getContent(BookPage page) throws ContentReadException {
		...
		String xmlContent;
		try {
			log.info("URL: " + page.getBook().getURL() + " Location: " + page.getLocation());
			;
			xmlContent = readTEIContent("file:///" + page.getBook().getURL(),
			        page.getLocation());
