		...
		try {
			...
			for (int i = 1; i <= len; i++) {
				...
				if (!pageTable.containsKey(IPageNr)) {
					...
					childPage.put("partTitle", "sz�mozatlan oldalak");
				...
				if(origPageNr == null) {
					childPage.put("origPageNr", "sz�mozatlan");
				}
				*/
		...
		try {
			...
			while (item != null) {
				...
				if(pageNr == -1 && parentName.trim().length() > 0){
					log.error("PDF hiba: 0-�s oldalsz�m: '" + title + "', '" + parentName + "', '" + path + "'");
				}
				...
				if(prefix.equals("parlamenti_naplo")) {
					if(year.indexOf("i") > -1) {
						...
						if(path.indexOf("Oldalsz�mok") > -1) {
							fieldName = "origPageNr";
						} else if(path.indexOf("�l�snapok") > -1) {
							fieldName = "ulesnap";
						} else {
					...
					if(path.indexOf("�VES TARTALOMJEGYZ�K") > -1 ||
					   path.indexOf("�VES TARTALOMJEGYZ��K") > -1)
					{
						fieldName = "title";
					} else {
						Pattern pattern = Pattern.compile(":[0-9]+\\. sz�m");
