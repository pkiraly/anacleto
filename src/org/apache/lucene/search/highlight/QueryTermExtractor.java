...
public final class QueryTermExtractor {
    ...
    private static final void getTerms(Query query, HashSet terms,
    	...
    	if (query instanceof BooleanQuery)
            ...
            getTermsFromPhrasePrefixQuery((PhrasePrefixQuery) query, terms);
    ...
    private static final void getTermsFromPhrasePrefixQuery(PhrasePrefixQuery query,
            HashSet terms) {
    	...
    	for (int i = 0; i < positions.length; i++) {
			...
			for (int j = 0; j < queryTerms.length; j++) {
	            terms.add(new WeightedTerm(query.getBoost(), queryTerms[j].text()));
	        }	
		}
