...
<xsl:stylesheet
  ...
  extension-element-prefixes="edate exsl"
  exclude-result-prefixes="edate exsl" 
...
<xsl:template match="/">
  <xsl:message>[teihtml-main.xsl:128] match="/"</xsl:message>
  ...
  <xsl:choose> 
    <xsl:when test="$makePageTable = 'true' and TEI.2">
      <xsl:message>[teihtml-main.xsl:138] $makePageTable = 'true' and TEI.2</xsl:message>
    ...
    <xsl:when test="not($ID='')">
      <xsl:message>[teihtml-main.xsl:177] not($ID='')</xsl:message>
    ...
    <xsl:when test="$masterFile= '' or $STDOUT='true'">
      <xsl:message>[teihtml-main.xsl:204] $masterFile= '' or $STDOUT='true'</xsl:message>
      <xsl:apply-templates/>
    </xsl:when>
  
    <xsl:when test="$splitLevel=-1"> 
      <xsl:message>[teihtml-main.xsl:209] $splitLevel=-1</xsl:message>
    ...
    <xsl:when test="TEI.2 or teiCorpus.2">
      <xsl:message>[teihtml-main.xsl:209] TEI.2 or teiCorpus.2</xsl:message>
      <xsl:apply-templates mode="split"/>
    </xsl:when>
  
    <xsl:when test="$makeFrames='true'">
      <xsl:message>[teihtml-main.xsl:209] $makeFrames='true'</xsl:message>
      <xsl:call-template name="doFrames"/>
    </xsl:when>
  
    <xsl:otherwise>
      <xsl:message>[teihtml-main.xsl:209] otherwise</xsl:message>
      <xsl:call-template name="outputChunk">
        <xsl:with-param name="ident">
          <xsl:value-of select="$masterFile"/>
        </xsl:with-param>
...
<xsl:template match="TEI.2">
   <xsl:if test="$verbose='true'">
     <xsl:message>[teihtml-main.xsl:238] TEI HTML: run start hook template teiStartHook</xsl:message>
   </xsl:if>
   ...
   <body>
     ...
     <a name="TOP"/>
     <xsl:comment>TEI.2::call stdheader</xsl:comment>
     ...
     <xsl:message>[teihtml-main.xsl:292] TEI.2->stdfooter</xsl:message>
...
<xsl:template match="TEI.2" mode="split">
  ...
  <xsl:call-template name="makeHash">
    <xsl:with-param name="content">
      <xsl:apply-templates mode="hash" select="text/body/div0" />
    </xsl:with-param>
  </xsl:call-template>
  <xsl:message>[teihtml-main.xsl:314] "TEI.2" mode="split hash OK</xsl:message>


  <xsl:variable name="BaseFile">
    <xsl:value-of select="$masterFile"/>
    <xsl:message>[teihtml-main.xsl:314] "TEI.2" mode="split->addCorpusID</xsl:message>
    <xsl:call-template name="addCorpusID"/>
  </xsl:variable>

  <xsl:if test="$verbose='true'">
    <xsl:message>[teihtml-main.xsl:316] TEI HTML: run start hook template teiStartHook</xsl:message>
  </xsl:if>

  <xsl:message>[teihtml-main.xsl:319] match="TEI.2" -> teiStartHook</xsl:message>
  <xsl:call-template name="teiStartHook"/>
  
  <xsl:if test="$verbose='true'">
    <xsl:message>[teihtml-main.xsl:323] TEI HTML in splitting mode, base file is '<xsl:value-of select="$BaseFile"/>' </xsl:message>
  </xsl:if>

  <xsl:message>[teihtml-main.xsl:319] match="TEI.2" -> outputChunk</xsl:message>
  <xsl:call-template name="outputChunk">
    <xsl:with-param name="ident">
      <xsl:message>[teihtml-main.xsl:329] p.indent='<xsl:value-of select="$BaseFile"/>'</xsl:message>
      <xsl:value-of select="$BaseFile"/>
    </xsl:with-param>
    <xsl:with-param name="content">
      <xsl:message>[teihtml-main.xsl:329] p.content='$writeMain'</xsl:message>
      <xsl:call-template name="writeMain"/>
    </xsl:with-param>
  </xsl:call-template>

  <xsl:if test="$verbose='true'">
    <xsl:message>[teihtml-main.xsl:339] END of splitting mode, base file was <xsl:value-of select="$BaseFile" /></xsl:message>
    <xsl:message>[teihtml-main.xsl:340] TEI HTML: run end hook template teiEndHook</xsl:message>
  </xsl:if>
...
<!--
  <xsl:message>[teihtml-main.xsl:353] htmlFileBottom->stdfooter</xsl:message>
  <xsl:call-template name="stdfooter">
    <xsl:with-param name="date">
      <xsl:call-template name="generateDate"/>
    </xsl:with-param>
...
<xsl:template name="htmlFileTop">
  <xsl:message>[teihtml-main.xsl:359] template htmlFileTop</xsl:message>
  <xsl:comment>THIS FILE IS GENERATED FROM AN XML MASTER. DO NOT EDIT ME</xsl:comment>
...
<xsl:template name="writeMain">
  <xsl:message>[teihtml-main.xsl:389] writeMain</xsl:message>
  <xsl:comment>[teihtml-main.xsl:387] writeMain template</xsl:comment>
  <html>
    ...
    <body>
      ...
      <xsl:call-template name="bodyJavaScript"/>
      <a name="TOP"/>
      <xsl:comment>writeMain::call stdheader</xsl:comment>
      <xsl:message>[teihtml-main.xsl:389] writeMain->stdheader</xsl:message>
      <xsl:call-template name="stdheader">
        <xsl:with-param name="title">
          <xsl:message>[teihtml-main.xsl:389] p.title: writeMain->generateTitle</xsl:message>
          <xsl:call-template name="generateTitle"/>
        </xsl:with-param>
      </xsl:call-template>
      ...
      <xsl:message>[teihtml-main.xsl:409] writeMain->htmlFileBottom</xsl:message>
...
<xsl:template name="stdheader">
  <xsl:message>[teihtml-main.xsl:474] stdheader</xsl:message>
  <xsl:message>[teihtml-main.xsl:475] p.$title=<xsl:value-of select="$title" /></xsl:message>
  ...
  <xsl:choose>
    <xsl:when test="$makeFrames='true'">
      ...
      <xsl:if test="$showTitleAuthor='true'">
        <xsl:if test="$verbose='true'">
          <xsl:message>[teihtml-main.xsl:478] stdheader->generateAuthorList, stdheader->generateDate (displaying author and date)</xsl:message>
        </xsl:if>
        <xsl:call-template name="generateAuthorList"/>
        <xsl:text> </xsl:text>
        <xsl:call-template name="generateDate"/>
    ...
    <xsl:otherwise>
      <h1><xsl:value-of select="$docMainTitle"/></h1>
<!--
      <table class="header" width="100%">
        <tr>
          <xsl:message>[teihtml-main.xsl:496] stdheader->logoPicture</xsl:message>
          <td rowspan="3"><xsl:call-template name="logoPicture"/></td>
          <td align="left">
        ...
        <tr>
          <td align="left">
            ...
            <xsl:comment>stdheader::<xsl:copy-of select="$title" /></xsl:comment>
            <h1 class="maintitle"><xsl:value-of select="$title"/></h1>
          </td>
        </tr>
        <xsl:if test="$showTitleAuthor='true'">
...
<xsl:template name="stdfooter">
  <xsl:message>[teihtml-main.xsl:532] stdfooter</xsl:message>
  <!-- <xsl:param name="date"/>
  ...
  <address>
    ...
    <xsl:comment>
      ...
      <xsl:text> stylesheet based on </xsl:text>
      <xsl:value-of select="$teixslHome"/>
      <xsl:text>teihtml.xsl processed using </xsl:text>
      <xsl:value-of select="system-property('xsl:vendor')"/>
  ...
  <xsl:if test="ancestor::teiCorpus">
    <p align="{$alignNavigationPanel}">
      ...
      <xsl:call-template name="upLink">
        ...
        <xsl:with-param name="u" select="$contentsWord" />
...
<xsl:template name="doBody">
  <xsl:message>[teihtml-main.xsl:623] doBody</xsl:message>
  <!-- <xsl:param name="Head"/>-->
  <xsl:message>[teihtml-main.xsl:625] p.Head <xsl:value-of select="$Head"/></xsl:message>
  ...
  <xsl:choose>
    ...
    <xsl:when test="parent::div/@rend='multicol'">
      <td valign="top">
        <xsl:if test="not($Head = '')">
          <xsl:message terminate="no">[teihtml-main.xsl:641] h{$Head + $divOffset}</xsl:message>
    ...
    <xsl:otherwise>
      <xsl:if test="not($Head = '')">
        <xsl:message>[teihtml-main.xsl:664] h{<xsl:value-of select="$Head"/> + <xsl:value-of select="$divOffset"/>}</xsl:message>
        <xsl:element name="h{$Head + $divOffset}">
          <a name="{$ident}"></a>
...
<xsl:template match="*" mode="xpath">
  <xsl:message>[teihtml-main.xsl:633] match="*"  mode="xpath"</xsl:message>
        ...
        <xsl:otherwise>
          <html>
            ...
            <body>
              ...
              <xsl:call-template name="bodyJavaScript"/>
              <a name="TOP"/>
              <xsl:comment>match="*" mode="xpath"::call stdheader</xsl:comment>
...
<!-- HASHELÉS -->
<xsl:template match="div0 | div1 | div2 | div3 | div4 | div5 | div6 | div7 | div8 | div9"  mode="hash">
  <xsl:text> </xsl:text>
  <xsl:element name="counter">
    ...
    <xsl:attribute name="count">
      <!--
      <xsl:value-of select="count( ancestor::*[starts-with(name(),'div')]
                         | preceding::*[starts-with(name(),'div')]
                         | self::*
                  )" / -->
      <!-- xsl:text>5-</xsl:text -->
      <xsl:number count="div0 | div1 | div2 | div3 | div4 | div5 | div6 | div7 | div8 | div9 | self" format="1_1_1_1_1_1_1_1_1" level="any" />
    </xsl:attribute>
  ...
  <xsl:text>&#xa;</xsl:text>
  <xsl:apply-templates select="child::div1|child::div2|child::div3|child::div4|child::div5|child::div6|child::div7|child::div8|child::div9" mode="hash" />
</xsl:template>
...
<xsl:template name="makeHash">
  ...
  <exsl:document         
        encoding="iso-8859-2" 
        method="xml"
        href="c:/perl/kiru/{$filenamePrefix}hash.xml"
        indent="yes">
