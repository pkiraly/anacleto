          ...
          <xsl:for-each select="@*">
            <xsl:choose>
              <xsl:when test="name(.) = 'vertical-align'">
                <xsl:variable name="my_valign"><xsl:value-of select="." /></xsl:variable>
                <xsl:choose>
                  <xsl:when test="$my_valign = 'bottom'">
                    <xsl:text>vertical-align: -2pt;</xsl:text>
                  </xsl:when>
                  <xsl:when test="$my_valign = 'top'">
                    <xsl:text>vertical-align: 2pt;</xsl:text>
                  </xsl:when>
                  <xsl:otherwise>
